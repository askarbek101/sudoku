...
public class SudokuPanel extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		g2d.setColor(new Color(1.0f,1.0f,1.0f));    // color white
		...
		for(int x = 0;x <= usedWidth;x+=slotWidth) {			//draw lines
		...
		for(int y = 0;y <= usedHeight;y+=slotHeight) { 			// draw lines
		...
		if(currentlySelectedCol != -1 && currentlySelectedRow != -1) {
			g2d.setColor(new Color(0.0f,0.0f,1.0f,0.3f)); //color of blocks
	...
	public void messageFromNumActionListener(String buttonValue) {
		if(currentlySelectedCol != -1 && currentlySelectedRow != -1) {
			puzzle.makeMove(currentlySelectedRow, currentlySelectedCol, buttonValue, SudokuFrame.count);
			repaint();
		}
	}
	...
	private class SudokuPanelMouseAdapter extends MouseInputAdapter {
		@Override
		public void mouseClicked(MouseEvent e) {			//cursor position when clicked
